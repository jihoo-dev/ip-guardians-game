...
const TILE_KINDS = {
  ...
  /* autoMs 3000 → 2500.
   * 강화 타일이 바닥 전체가 되면서 층이 죽는 속도를 이 값 하나가
   * 결정하게 됐습니다. 종전 일반 타일은 밟고 0.7~1.0초였으니 3초는
   * 3배 이상 느립니다. 2.5초면 '한 번 지나간 자리를 다시 지날 수는
   * 있지만 오래는 못 버틴다' 정도로, 층이 제시간에 정리됩니다. */
  reinforced: { hits: 2, fuseMin: 0,    fuseMax: 0,    autoMs: 2500 },
...
const SPECIAL_TIPS = {
  ...
  reinforced: '바닥은 한 번 밟으면 금이 갑니다 — 한 번 더 밟히거나 잠시 지나면 무너집니다',
  accel:  '신속심사 타일 — 진행 방향으로 두 칸 튀어나갑니다. 앞이 비어 있으면 그대로 떨어집니다',
  reject: '거절이유 발견 — 잠시 뒤 이 타일과 맞닿은 6칸이 함께 무너집니다. 지금 벗어나세요',
...
const LAYER_DEFS = [
  ...
  { key: 'fasttrack', name: '우선심사 패스트트랙', short: 'FAST-TRACK',
    ...
    tip: '밝게 빛나는 타일을 밟으면 진행 방향으로 두 칸 튀어나갑니다',
    marks: [{ i: '⚡', t: '밝게 빛나는 타일 = 앞으로 두 칸 사출' }] },
  ...
  { key: 'exam',      name: '실체심사/선행기술조사', short: 'EXAMINATION',
    color: '#7c5cff', special: 'none',   trait: 'none',
    tip: '특별한 장치가 없는 층입니다 — 금 간 타일만 피해 달리세요',
    marks: [{ i: '—', t: '장치 없는 층 · 금 간 타일만 피하면 됩니다' }] },
  { key: 'office',    name: '의견제출통지',       short: 'OFFICE ACTION',
    color: '#0088ff', special: 'reject', trait: 'rotate',
    tip: '층 전체가 천천히 돕니다 · 노랗게 변한 타일은 곧 주변까지 터집니다',
    marks: [
      { i: '⟳', t: '층 전체가 회전 · 15초마다 방향 반전' },
      { i: '◆', t: '노랗게 변한 타일 = 주변 6칸까지 폭발' }
    ] },
  /* ── 보정 층 (2026-09-13 추가) ────────────────────────────────────
  ...
  { key: 'amend',     name: '보정',               short: 'AMENDMENT',
    ...
    tip: '이 층도 장치가 없습니다 — 숨을 고르고 다음 층을 고르세요',
    marks: [{ i: '—', t: '장치 없는 층 · 숨 고르고 다음 층을 고르세요' }] },
  { key: 'claim',     name: '청구항',             short: 'CLAIM',
    color: '#ff8a3d', special: 'phase',  trait: 'none',
    tip: '깜빡이는 타일은 꺼져 있는 동안 밟을 수 없습니다',
    marks: [{ i: '◇', t: '깜빡이는 타일 = 꺼진 동안 밟을 수 없음' }] },
  { key: 'idea',      name: '아이디어',           short: 'IDEATION',
    color: '#aa00ff', special: 'jump',   trait: 'none',
    tip: '청록·자홍 발판을 밟으면 위층으로 다시 올라갑니다',
    marks: [{ i: '▲', t: '청록 = 한 층 위 · 자홍 = 두 층 위' }] }
];
...
const LAYERS = LAYER_DEFS.map((d, i) => ({
  ...
  special: d.special,
  trait: d.trait,
  /* 그 층에 처음 내려섰을 때 띄우는 한 줄 안내. 클라이언트가 문구를
   * 들고 있으면 층 구성을 바꿀 때마다 두 파일을 고쳐야 합니다.     */
  tip: d.tip || '',
